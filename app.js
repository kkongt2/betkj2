...
function chart(items,label='분리 평가 기간의 일별 누적 세전 손익'){
  ...
  const vals=[0,...items.map(x=>x.profit_units*Tuning.STAKE)],lo=Math.min(...vals),hi=Math.max(...vals),span=hi-lo||1;
