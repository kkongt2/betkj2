...
const context=vm.createContext({document:{getElementById:get,createElement:()=>new Element()},Date:FixedDate,Intl,console,setInterval:()=>{},Tuning:require('../tuning.js'),QPL:require('../model.js'),fetch:async path=>({ok:true,json:async()=>JSON.parse(fs.readFileSync(path.split('?')[0]==='data/latest.json'?'tests/ui-fixture.json':path.split('?')[0],'utf8'))})});
...
setImmediate(()=>{
  ...
  assert.match(get('metrics').innerHTML,/261,000원/);assert.match(get('validation').innerHTML,/최고 수익일/);
