...
try{
 if(msg.type==='load'){
  ...
  self.postMessage({type:'ready',races:data.races.length,candidates:data.races.reduce((s,r)=>s+r.rows.length,0),from:data.from,through:data.through});
 }else if(msg.type==='robust'){
  if(!data)throw Error('자료를 먼저 불러와 주세요.');
  const report=Robust.run(data,msg.settings,p=>self.postMessage({type:'robust-progress',id:msg.id,...p}));
